    ...
    return Scaffold(
      ...
      body: GetBuilder<CreateInvoiceController>(
        builder: (controller) {
          ...
          if(!isCompleted){
            return Center(
              child: Text('Please complete your first'),
            );
          }
          return ListView.separated(
              itemCount: controller.invoiceCreateResponseModel?.paymentMethod?.length??0,
              itemBuilder: (context,index){
                final PaymentMethod paymentMethod = controller.invoiceCreateResponseModel!.paymentMethod![index];//kokono null hobe na
                return ListTile(
                  leading: Image.network(paymentMethod.logo??'',width: 60,),
                  title: Text(paymentMethod.name ?? ''),
                  onTap: (){
                    Get.off(()=>WebView(paymentUrl:paymentMethod.redirectGatewayURL!,));
                  },
                );
              },
